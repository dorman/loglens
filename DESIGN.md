...
loglens is a dark, evenly-lit field that you hold a log up against so its anomalies glow on their own. The ground is deliberately underexposed — a muted one-dark graphite — not because dark terminals are fashionable, but because tint only carries meaning when nothing around it competes. An `INFO` line recedes into the ground. A `WARN` line warms. An `ERROR` line glows. That gradient is the primary reading instrument, and it works before the user has typed a single keyword. The neutral steps descend in luminance — Paper White `.65` for an unclassified line, Status Quartz `.44` for one known to be routine, Slate Mute `.23` for debug noise — so "recedes" is a measured relationship, not a manner of speaking. `INFO` deliberately does *not* wear Signal Blue: spending the accent on the most numerous line in any log diluted the one value that means "reachable or currently yours".
...
**The Level-Is-A-Field Rule.** A level is a field in the log's own format, and every format puts it in the prefix. `detect_level` therefore takes the *first* level token in a line, never the loudest. The vocabulary spans syslog (`ALERT`/`EMERG`/`NOTICE`), Java util.logging (`SEVERE`/`CONFIG`/`FINE`…) and Windows event logs (`Information`) as well as the common application levels — but anything above `ERROR` still maps to the Error tint, because the scale is four steps by commitment and a fifth would need a fifth color. Taking the maximum instead let message text repaint the line: Android's `SYSTEM_ALERT_WINDOW` permission tokenises to `SYSTEM`/`ALERT`/`WINDOW` and painted every `INFO` line carrying it as an error.
