...
- Key first moves once open: `S` scan for known-bad signatures, `a` add keyword highlight, `/` search, `f` filter, `:` go to line, `m` bookmark (`M` clear), `y` yank, `e` export findings, `?` help, `q` quit.
