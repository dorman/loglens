...
- A scan starts automatically on open (`--no-scan` opts out); `S` rescans. Other first moves: `a` add keyword highlight, `/` search, `f` filter, `:` go to line, `m` bookmark (`M` clear), `←`/`→` pan long lines, `y`/`Y` yank line/path, `e` export findings, `?` help, `q` quit.
